...
\vspace{1cm}
\begin{center}
    \includegraphics[width=.5\linewidth]{foto_1.png}
\end{center}
\vspace{2cm}
...
Era una madrugada en Cordoba, cuando suena el telefono en la mesada. Al atender, Guido 




En este trabajo se comentara cuales son los pasos que Guido realiza para poder determinar la hora de muerte de Sergio.


## Paso 1: Entender la metodologia
...
donde $T_{amb}$ es la temperatura ambiente, $r$ una constante positiva que indica la rapidez del *algor mortis* y $T(t)$ es la función que describe la temperatura del cuerpo en función del tiempo.
...
Si $T_{amb}=23$ y $T_{ini}=23$ la forma de la función es la siguiente:
...
Para simplificar el problema, no se utiliza la función $T(t)$ sino la $ln(T(t)-T_{amb})$ ya que esta tiene forma lineal.

$$T(t)-T_{amb} = T_{diff}e^{-rt}$$
Donde: $T_{diff} = T_i-T_{amb}$

$$ln(T(t)-T_{amb}) = ln(T_{diff}e^{-rt}) = ln(T_{diff})+ln(e^{-rt}) = ln(T_{diff})-rt$$

Dicha función esta representada por la ecuacción de una recta $\beta_0+\beta_1*x$
donde la ordenada al origen representa . 
