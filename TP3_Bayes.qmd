...
# Capítulo 2: Además de que está muerto, ¿Qué sabemos?
...
$$Y_i = ln(T(t_i)-T_{amb})$$
$$\beta_0 = ln(T_{ini}-T_{amb}) = ln(T_{diff})$$
$$\beta_1 = -r$$
...
Al investigar sobre el error de medición para la temperatura, se encuentra que el error máximo de un termómetro es de 0.7 ºC, entonces se calcula la diferencia entre el logaritmo natural de una temperatura $T$ y la temperatura con su error máximo:
$$ln(T+0.7)-ln(T) = ln(\frac{T+0.7}{T}) = ln(1+\frac{0.7}{T})$$
Y esta expresión será máxima cuando T sea mínimo, es decir, $T=22$. Entonces,
```{r, echo = FALSE}
...
ggplot(df_sigma) + geom_line(aes(x = x, y = y)) + scale_x_continuous(name = "Sigma", breaks = round(seq(0,sigma_max,length.out = 4),3))  + scale_y_continuous(name = "Densidad") +
  theme_bw()
```
Donde dichos valores representan el porcentaje
